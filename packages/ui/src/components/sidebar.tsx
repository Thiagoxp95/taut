...
import { PanelLeftIcon } from '@taut/ui/components/icons'
import { cn } from '@taut/ui/lib/utils'
import { useIsMobile } from '@taut/ui/hooks/use-mobile'
...
function SidebarProvider({
  ...
  // Uncontrolled by default; `openProp`/`setOpenProp` make it controlled.
  const [_open, _setOpen] = React.useState(defaultOpen)
  const open = openProp ?? _open
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const openState = typeof value === 'function' ? value(open) : value
      if (setOpenProp !== undefined) setOpenProp(openState)
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* The gap that pushes the page over; the sidebar itself is fixed. */}
      <div
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'taut-scroll flex min-h-0 flex-1 flex-col gap-2 overflow-x-hidden overflow-y-auto',
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      {asChild ? children : <span data-slot="sidebar-group-label-text">{children}</span>}
    </Comp>
  )
}
...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring transition-transform outline-hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm whitespace-nowrap ring-sidebar-ring outline-hidden focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring transition-transform outline-hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground peer-hover/menu-button:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        // Increase the hit area on touch devices.
        'after:absolute after:-inset-2 md:after:hidden',
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  ...
  return (
    <div data-slot="sidebar-sub-disclosure" inert={!isMobile && state === 'collapsed'}>
      <div data-slot="sidebar-sub-clip">
        <ul
