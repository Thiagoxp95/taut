...
export const tautSection = (input: {
  ...
  const tools = input.mcpAvailable
    ? [
        ...
        '- Only confirm a timer started after render_component succeeds with signalId and endsAt. For validation errors, correct the indicated arguments once; if it still fails, stop and give one brief, plain-language explanation (for example, "I couldn’t start the timer."). Do not retry equivalent inputs, paste schema traces into chat or cards, or claim a platform bug without evidence. If a requested component or behavior is unsupported, say what you cannot do and offer a supported alternative. For an uncertain network result, check list_signals before retrying so you do not create duplicate timers.',
