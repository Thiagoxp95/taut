...
/** A thread meter supplies occupancy to its avatar without assigning global agent usage. */
 ...
 * A thread's context meter can request the same orb while that session compacts.
 * Sidebar rows, member lists, profile cards and pickers stay a face however busy
 * the agent is.
 *
...
export function AvatarVisual({
  ...
  presence?: Presence
  /**
   * Agents only: morph into the thinking orb for the reply being written.
   * Session compaction requests the same animation through `compacting`.
   */
  working?: boolean
  /** Agents only: 0–100 context occupancy; expands the silhouette into a circle. */
  contextPercentage?: number
  /** Agents only: break into the loading orb while the runtime compacts. */
  compacting?: boolean
  ...
  const isCompacting = compacting ?? context.compacting ?? false
    ...
    <span className={cn('relative inline-flex shrink-0', className)}>
      {kind === 'agent' ? (
        <AgentFigure
          ...
          shape={face?.shape}
          state={orb}
          working={working || isCompacting}
          contextPercentage={contextPercentage ?? context.contextPercentage}
          px={PX[size]}
        />
      ) : (
        <span
