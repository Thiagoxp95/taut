...
const info = (() => {
  const raw = ipcRenderer.sendSync('taut:info')
...
const taut = {
  ...
  setBadge: (count) => ipcRenderer.send('taut:badge', count),
  notify: (notification) => ipcRenderer.send('taut:notify', notification),
  allowMediaOrigin: (url) => ipcRenderer.send('taut:media-origin', url),
  openHuddle: (path) => ipcRenderer.send('taut:huddle:open', path),
