 ...
 */
export const normalizeInstanceUrl = (raw: string): Either.Either<string, InvalidInstanceUrl> => {
...
export const socketUrl = (instanceUrl: string, since: number): string => {
  const url = new URL('/ws', instanceUrl)
  url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
  // No stored cursor means there is nothing to catch up on: omitting `since` asks the
  // server for the head (`resync`) instead of a replay of the whole log.
...
export const probeInstance = (
  origin: string
): Effect.Effect<{ readonly version: string }, InstanceUnreachable> =>
  Effect.tryPromise({
    try: (signal) =>
