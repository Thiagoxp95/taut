...
const EMPTY: DesktopState = { lastSeq: 0 }
...
export class Store extends Effect.Service<Store>()('Store', {
  effect: Effect.gen(function* () {
    ...
    const loaded = yield* Effect.sync(() => {
      try {
        return decode(JSON.parse(readFileSync(file, 'utf8')) as unknown)
      } catch {
        return Option.none<DesktopState>()
      }
    })
    const ref = yield* Ref.make(Option.getOrElse(loaded, () => EMPTY))

    const flush = (state: DesktopState) =>
      Effect.try(() => {
    ...
    const update = (f: (state: DesktopState) => DesktopState) =>
      Ref.updateAndGet(ref, f).pipe(Effect.tap(flush))
